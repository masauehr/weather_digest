...
ローカルLLM 4種（Ollama: qwen3.6 / ornith-1.5 / nemotron-3.5-lightning / qwen3.8）と Claude Haiku の
計5モデルで同じ週を記事化し、Claude Sonnet が評価した比較ページを自動生成する。
...
`local_agent.py` は `--slug`（qwen / ornith / nemotron / qwen38）でエンジンを切り替える。
ornith / nemotron / qwen38 は「secondary エンジン」で、README・トップ index.md は更新せず
自分のアーカイブ一覧（`articles/<slug>_weekly/index.md`）だけを更新する。
`generate_compare.py` は qwen と Haiku を必須とし、存在する記事だけを N 枚のパネルに並べる。

【2026-09-22 変更】比較対象にローカルLLM `qwen3.8:27b-mlx` を追加（`--slug qwen38`、日曜11:00）。
あわせて secondary エンジン共通の2件のバグを修正:
  - `_insert_li_at_top_of_ul` に重複挿入防止チェックを追加（`update_index` の二重呼び出しでアーカイブ
    一覧に同一行が重複していた問題。`articles/ornith_weekly/index.md` 等で実際に発生していたものを修正）
  - secondary の月次システムプロンプトに `update_index` の `month_label`/`month_path` 引数を明記
    （従来は月次モードでも week 引数しか案内しておらず、モデルが月次エントリを誤って週次アーカイブに
    間違った href で登録する不具合があった）
...
| Ollama（qwen3.8:27b-mlx） | 毎週日曜 11:00 | `run_weather_qwen38.sh` → `local_agent.py --slug qwen38` | `articles/qwen38_weekly/` |
| Claude Haiku（Claude Code CLI） | 毎週日曜 12:00 | `run_weather_haiku.sh` → `haiku_agent.py` | `articles/haiku_weekly/` |
| 各ローカルLLM 月次 | 毎月第1日曜 08:00〜11:00 | 上記各 run スクリプト（monthly モード） | `articles/<slug>_monthly/` |
...
| qwen3.8 週次記事 | `articles/qwen38_weekly/YYYY-MMDD.md` |
...
| qwen3.8 月次記事 | `articles/qwen38_monthly/YYYY-MM.md` |
...
| qwen3.8 ログ | `weather_digest_qwen38.log` |
...
# 比較ページのみ手動生成（qwen と Haiku の記事が揃っている場合。ornith / nemotron / qwen38 はあれば追加される）
