#!/bin/bash
# run_weather_qwen38.sh — Ollama（qwen3.8:27b-mlx）による気象ニュース週次まとめ 自動実行スクリプト
# launchd から毎週日曜 11:00 に呼び出される（qwen3.6版08:00・ornith版09:30・nemotron版10:30 の後、Haiku版12:00 の前）。
# モデル比較用の追加ローカルLLM。README・トップ index.md は更新せず、
# 自分のアーカイブ一覧（articles/qwen38_weekly/index.md）だけを更新する。
