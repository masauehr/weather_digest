#!/bin/bash
# run_weather_ornith.sh — Ollama（ornith-1.5:35b）による気象ニュース週次まとめ 自動実行スクリプト
# launchd から毎週日曜 09:30 に呼び出される（qwen版 08:00 の後、Haiku版 12:00 の前）。
# モデル比較用の追加ローカルLLM。README・トップ index.md は更新せず、
# 自分のアーカイブ一覧（articles/ornith_weekly/index.md）だけを更新する。
...
OLLAMA_MODEL="${OLLAMA_MODEL:-ornith-1.5:35b}"
